...
좀 더 일반적으로 어떤 수가 $\pmod n$에 속할 수 있는 가능한 최고 지수는 $\phi(n)$이라고 할 수 있다. 만약 어떤 수가 이러한 차수에 속한다면 그것은 $n$의 *primitive root*라고 한다. 여기서 중요한 것은 만약 $a$가 $n$의 *primative root*라면 그것의 거듭 제곱 $a, a^2, ..., a^{\phi(n)}$은 $\pmod n$에서 서로 distinct하고 모두 $n$의  *primatively root*이다. 특히 prime $p$이고, $a$가 $p$의 *primitive root*인 경우에 거듭 제곱 $a, a^2, ..., a^{p-1}$은 서로 $\pmod p$에서 distinct하다. e.g. $p=19$,  *primatively root*는 $2, 3, 10, 13, 14, 15$이다 
